...
namespace ConsumerApp
{
    public partial class _Default : System.Web.UI.Page
    {
        ...
        protected void getData_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (var stream = response.GetResponseStream())
                {
                    using (StreamReader reader = new StreamReader(stream))
                    {
                        ...
                        {
                            accountListBox.Items.Add(String.Format("{0} - {1} - {2} - {3}", dataObject[i]["Title"], dataObject[i]["Iban"], dataObject[i]["Balance"], dataObject[i]["Currency"]));
                        }
                        dataLabel.Text = data;
                    }
                }
            ...
            {
                OutputLabel.Text = "Could not obtain data, the application probably does not have access to the data any more";
            }
        }
